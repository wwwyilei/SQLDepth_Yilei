...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        frame_index, side = self.index_to_folder_and_frame_idx(index)

        poses = {}
        if type(self).__name__ in ["CityscapesPreprocessedDataset", "CityscapesEvalDataset","SelfvideoDataset"]:
            inputs.update(self.get_colors(frame_index, side, do_flip))
        else:
        ...
        if self.load_depth and False:
            depth_gt = self.get_depth( frame_index, side, do_flip)
            inputs["depth_gt"] = np.expand_dims(depth_gt, 0)
            inputs["depth_gt"] = torch.from_numpy(inputs["depth_gt"].astype(np.float32))

        return inputs

    def get_color(self, frame_index, side, do_flip):
        raise NotImplementedError

    def check_depth(self):
        raise NotImplementedError

    def get_depth(self, frame_index, side, do_flip):
        raise NotImplementedError
